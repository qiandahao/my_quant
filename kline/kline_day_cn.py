...
def get_stock_cn_list():
    ...
    url2 = 'https://stock.xueqiu.com/v5/stock/screener/quote/list.json?size=100&order=desc&order_by=symbol&market=US&type=sh_sz'
...
def generate_stock_csv():
    ...
    for index, row in df.iterrows():
        ...
        # 将DataFrame保存为CSV文件
        df.to_csv(row['symbol'] + '.csv', index=False)
...
threads = []
for _ in range(10):
    thread = threading.Thread(target=get_stock_cn_data)
