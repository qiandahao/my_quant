...
def generate_stock_csv():
    with open('./2024_05_28_CN/CN2024_05_28.json', 'r') as file:
        data = json.load(file)
    symbols = [item['symbol'] for item in data['data']['list']]
    ...
    for index, row in df.iterrows():
        print(index)
        if not os.path.exists('./2024_05_28_CN/'+ row['symbol'] + '.json'):
            continue
        json_data = pd.read_json('./2024_05_28_CN/'+ row['symbol'] + '.json')
